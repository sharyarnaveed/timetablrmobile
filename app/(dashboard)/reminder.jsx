...
const EventsPage = () => {
  ...
  const validateField = useCallback(
    async (fieldName, value) => {
      try {
        await eventSchema.validateAt(fieldName, {
          ...formData,
          [fieldName]: value,
        });
        setFormErrors((prev) => {
    ...
    try {
      if (role === "teacher") {
        ...
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_API_URL}/api/user/getreminder`,
          {
            headers: {
  ...
  const handleAddEvent = useCallback(async () => {
    ...
    try {
      ...
      if (role === "teacher") {
        ...
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_API_URL}/api/user/addreminder`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleCloseModal = useCallback(() => {
    const hasData = Object.values(formData).some(
      (value) => value !== "" && value !== "assignment" && value !== "medium",
    );
